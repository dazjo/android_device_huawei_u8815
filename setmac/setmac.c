...
void SetMAC(void)
{
	...
	char wlan_arg[PROPERTY_VALUE_MAX];
	char mac_bits[8];
	int y = 0;

	memset(mac_bits, 0, 8);

	/* Get the MAC address from the radio. */
	...
	/* Add the MAC address to existing module arguments. */
	sprintf(wlan_arg, "%s %s", DRIVER_MODULE_ARG, wlan_mac_arg);
	property_set(DRIVER_PROP_MODULE_ARG, wlan_arg);
}
